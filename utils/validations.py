...
def validate(data_loader, model, args):
    ...
    with torch.no_grad():
        ...
        for i,   (images, target) in enumerate(data_loader):
            ...
            # compute output
            with autocast():
                output, text_output = model(images)
            # output = Softmax(output).cpu()[:, 1]
            if output.dim() == 3:
                output = Softmax(output).cpu()[:, 1]
                # output += args.asl_pos_ratio * text_output.cpu()
            else:
...
def validate_zsl(data_loader, model, args, cls_id, is_unseen=False):
    batch_time = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i,   (images, target) in enumerate(data_loader):
            ...
            with autocast():
                ...
                output, _ = model(images, cls_id)
                output2, _ = model.forward_biased_class(images, cls_id)

            target = target[:, cls_id]
            # sum_target[cls_id] += target.sum(dim=0)
            if output.dim() == 3:
                # output[:, 1] += args.text_logits_ratio * text_output
                if is_unseen:
                    output = Softmax(output).cpu()[:, 1]
                    output2 = Softmax(output2).cpu()[:, 1]
                    output = output.float()
                    output2 = output2.float().cpu()
