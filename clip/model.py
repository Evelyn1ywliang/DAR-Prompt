...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x, instance_tokens=None):
        
        b, c, h, w = x.shape
        image_feat = x.reshape(b, c, h * w).permute(2, 0, 1).to(x.dtype)

 
        # image_feat = image_feat.reshape(image_feat.shape[0], image_feat.shape[1], image_feat.shape[2] * image_feat.shape[3]).permute(2, 0, 1)  # NCHW -> (HW)NC
        # image_feat = torch.cat([image_feat.mean(dim=0, keepdim=True), image_feat], dim=0)  # (HW+1)NC
        # image_feat = image_feat + self.positional_embedding[:, None, :].to(image_feat.dtype)  # (HW+1)NC
        image_feat = F.linear(image_feat, self.v_proj.weight, self.v_proj.bias)
        image_feat = F.linear(image_feat, self.c_proj.weight, self.c_proj.bias)       
        ...
        return x, attn_weights, image_feat
...
class AttentionConv(nn.Module):
    ...
    def forward(self, x, learnable_tokens=None):
        _, _, h, w = x.shape
...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, tokens=None, is_fc=False):
        def stem(x):
        ...
        if not is_fc:
            x = self.final_pool(x)
            x = self.attnpool(x, tokens)  # output: [50, 32, 1024]
        return x
