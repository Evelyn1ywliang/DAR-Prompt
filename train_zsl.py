...
from utils.trainers import train_coop_text_image
...
def main():
    ...
    log_folder = os.path.join(cfg.OUTPUT_DIR, arch_name, formatted_datetime)
    ...
    if args.resume is not None:
        if os.path.exists(args.resume):
            ...
            try:
                prompt_params = model.only_pos_neg_prompt_params()
            except:
                prompt_params = model.module.only_pos_neg_prompt_params()
            prompt_group = {'params': prompt_params} 
            sgd_polices = [prompt_group]
            ctx_evi_group = {'params': [model.prompt_learner.ctx_evi], 'lr': 0.0005}
            sgd_polices.append(ctx_evi_group)
            ...
            sched = build_lr_scheduler(optim, cfg.OPTIM) # ConstantWarmupScheduler
    ...
    prompt_group = {'params': prompt_params} 
    sgd_polices = [prompt_group]
    ctx_evi_group = {'params': [model.prompt_learner.ctx_evi], 'lr': 0.001}
    # sgd_polices = [ctx_pos_group, ctx_neg_group, ctx_evi_group]
    sgd_polices.append(ctx_evi_group)

    print("Add the temperature and spatial")
    for name, param in model.named_parameters():
        if "spatial_T" in name:
            param.requires_grad_(True)
            sgd_polices[0]['params'].append(param)
    ...
    for epoch in range(args.start_epoch, cfg.OPTIM.MAX_EPOCH):
        batch_time, losses, mAP_batches = train_coop_text_image(train_loader, [val_unseen_loader, val_gzsi_loader], model, optim, sched, args, cfg, epoch,  cls_id, logfile=logfile)
        print('Train: [{0}/{1}]\t'
        ...
        if (epoch + 1) % args.val_every_n_epochs == 0 or epoch == args.stop_epochs - 1:
            p_unseen, r_unseen, f1_unseen, mAP_unseen = validate_zsl(val_unseen_loader, model, args, val_unseen_cls_id, is_unseen=True)
